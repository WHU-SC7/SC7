...
/*渐变色，简单版本。手动添加*/
int add_figlet_char_with_color(char c,char *color) //< color必须是"\033[31;1m"这样的控制颜色序列
{
  int i;
  for(i=0;i<256;i++)//查找
  {
  ...
  {
    /*添加颜色*/
    memmove(figlet_big[j]+figlet_lens,color,strlen(color));
  }
  figlet_lens += strlen(color); //< 前进strlen(color)列
...
int add_figlet_char_auto_with_color(char c) //< color必须是"\033[31;1m"这样的控制颜色序列
{
  ...
  char *color=color_style[color_index]; 
