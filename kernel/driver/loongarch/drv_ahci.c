...
//替换为SC7的函数
void ahci_mdelay(uint32_t ms) //简单延时
{
    ms *= 100;
    while(ms>0)
        ms--;
...
{
    return (uint64_t)pmem_alloc_pages(1);
}

uint64_t ahci_phys_to_uncached(uint64_t va) //使用窗口1,访问设备寄存器
{
    return va | dmwin_win1;
}

uint64_t ahci_virt_to_phys(uint64_t va) //使用窗口0,访问内存，可能是栈上变量
{
...
/*ahci_printf, ahci_memset, ahci_memcpy函数直接去掉前缀名，这些函数在SC7有同名的*/
...
{
    [[maybe_unused]]uint64_t host_mmio = ahci_dev->mmio_base;
...
int ahci_host_init(struct ahci_device *ahci_dev)
{
    ...
    for (uint8_t i = 0; i < ahci_dev->n_ports; ++ i)
    {
        ...
        if (timeout <= 0)
            printf("port %u sata link timeout\n", i);
        else
            printf("port %u sata link up\n", i);
                      ...
                      uint32_t is_write)
{
    ...
    if (cmd_slot == 32)
    {
        printf("cannot find empty command slot\n");
        return 0;
    }
    ...
    {
        printf("max transfer length is %u bytes\n", AHCI_MAX_BYTES_PER_TRANS);
        return 0;
    }

    memcpy((void *)pp->cmd_tbl, cfis, sizeof(struct sata_fis_h2d));
...
int ahci_port_start(struct ahci_device *ahci_dev, uint8_t port)
{
    ...
    //printf("rx_fis = 0x%016lx, rx_fis_dma = 0x%016lx\n",
    ...
    //printf("cmd_tbl = 0x%016lx, cmd_tbl_dma = 0x%016lx\n",
    //        pp->cmd_tbl, pp->cmd_tbl_dma);
    mem += AHCI_CMD_TBL_HDR_SZ;
    pp->cmd_tbl_sg = (struct ahci_sg *)mem;
    //printf("cmd_tbl_sg = 0x%016lx,\n", pp->cmd_tbl_sg);
    ...
    if (timeout <= 0)
    {
        printf("ahci port %u failed to start\n", port);
        return -1;
    }
...
void ahci_sata_scan(struct ahci_device *ahci_dev)
{
    ...
    memcpy(pdev->product, product, sizeof(product));

    // serial number
    ata_id_c_string(id, serial, ATA_ID_SERNO, sizeof(serial));
    memcpy(pdev->serial, serial, sizeof(serial));
...
{
    ahci_sata_read_common(&_ahci_dev,block_num,1,buf);
}

void sata_write(void *buf, uint64 block_num)
{
    ahci_sata_write_common(&_ahci_dev,block_num,1,buf);
}
