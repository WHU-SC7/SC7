...
# 2025.8.14 lm
[feat] 2K1000磁盘读写
1. 使用官方的磁盘驱动，可以正常读写. 移植驱动时进行了函数替换，虽然对一些函数有些疑惑，但是能跑了
2. 开启对齐检查,对齐异常未处理，只会panic

[feat] 2K1000适配文件系统
1. 可以在板子上识别目录，创建console设备文件。
2. 执行程序有问题，板子上不能执行程序。qemu 子进程执行完后切换到父进程异常.
3. 读取字符问题。uart_getc在板子跑不了，但是在qemu -M ls2k可以跑